...
define([
   ...
   * The original field gets hidden, and a visible copy of it is appended just
   * on its place. The foreground field's role is to communicate with a user,
   * while the hidden (original) one is the one which is going to be sent 
   * on submit.
 	 */
 	function SynchronizedField($element, options) {

    options = $.extend({}, SynchronizedField.defaults, options);
